 ...
 *          WDT:      Bootloader WDT ch0 feed only -- no install/setup.
 *
 ...
 *          Original fix treated -EALREADY/-EBUSY/-ENOTSUP as "already
 *          running, fall back to ch0". See WDT field fix note below for
 ...
 *          Heap base (0x2000B8B0) is safely above all .noinit and stack
 *          regions.
 *
 ...
 *          trinity_log_dump_previous() -- bench only, call before WDT init.
 *          RTT is available immediately at boot so the dump can run early.
 *          trinity_log_dump_previous_deferred() -- field/USB only, call
 *          AFTER trinity_wdt_init() and trinity_wdt_kick() so the WDT is
 *          fed before the flash partition scan begins. UART is not available
 *          until after kernel init completes so the dump must be deferred.
 *          trinity_wdt_kick() is called inside find_write_offset() during
 *          the scan to prevent WDT expiry on large partitions.
 *
 ...
 *          Root cause: the nRF WDT peripheral latches on first arm and any
 *          subsequent wdt_install_timeout() call triggers a hardware reset
 *          regardless of the Zephyr driver return value.
 *          wdt_setup() has the same problem -- returns -EPERM and resets.
 *          CONFIG_WDT_DISABLE_AT_BOOT is silently ignored on nRF52840
 *          because HAS_WDT_DISABLE_AT_BOOT is never selected by the nRF
 *          WDT driver Kconfig.
 *          Fix: trinity_wdt_init() never calls wdt_install_timeout() or
 *          wdt_setup() on nRF52840. In all modes (bench, field, USB) it
 *          grabs wdt0, assumes channel 0 (bootloader always uses ch0),
 *          feeds immediately, and returns. The main loop kick keeps it
 *          alive. This is identical behaviour to the bench fix -- the
 *          field path just never got the same treatment because it was
 *          silently resetting rather than halting.
 *
 * \note    RESETREAS bitmask fix (2026-03-31):
 *          Trinity originally had incorrect RESETREAS bit definitions.
 *          Corrected against Nordic nRF52840 MDK bitfields header:
 *            ~/zephyrproject/modules/hal/nordic/nrfx/mdk/nrf52840_bitfields.h
 *          Authoritative defines (verbatim from MDK):
 *            POWER_RESETREAS_RESETPIN_Pos (0UL) -- pin reset
 *            POWER_RESETREAS_DOG_Pos      (1UL) -- watchdog reset
 *            POWER_RESETREAS_SREQ_Pos     (2UL) -- soft reset
 *            POWER_RESETREAS_LOCKUP_Pos   (3UL) -- CPU lockup
 *            POWER_RESETREAS_OFF_Pos     (16UL) -- wake from System OFF via GPIO
 *            POWER_RESETREAS_LPCOMP_Pos  (17UL) -- wake from System OFF via LPCOMP
 *            POWER_RESETREAS_DIF_Pos     (18UL) -- wake from System OFF via debug
 *            POWER_RESETREAS_NFC_Pos     (19UL) -- wake from System OFF via NFC
 *            POWER_RESETREAS_VBUS_Pos    (20UL) -- wake from System OFF via VBUS
 *          BROWNOUT is NOT a reset cause on nRF52840 -- no such bit exists.
 *          Previous Trinity definition of TRINITY_RESET_BROWNOUT=(1u<<0) was
 *          wrong -- bit 0 is RESETPIN, not brownout.
 *          0x00000001 = RESET_PIN (pin reset button pressed).
 *          0x00000004 = WATCHDOG (WDT timeout).
 *          0x00000002 = DOG (watchdog -- same as above, raw bit).
 ******************************************************************************/
...
#define TRINITY_RESET_PIN       (1u << 0)   /* RESETPIN -- pin reset        */
#define TRINITY_RESET_DOG       (1u << 1)   /* DOG -- watchdog reset        */
#define TRINITY_RESET_SREQ      (1u << 2)   /* SREQ -- soft reset           */
#define TRINITY_RESET_LOCKUP    (1u << 3)   /* LOCKUP -- CPU lockup         */
#define TRINITY_RESET_OFF       (1u << 16)  /* OFF -- wake from System OFF  */
#define TRINITY_RESET_LPCOMP    (1u << 17)  /* LPCOMP -- wake from LPCOMP   */
#define TRINITY_RESET_DIF       (1u << 18)  /* DIF -- debug interface wake  */
#define TRINITY_RESET_NFC       (1u << 19)  /* NFC -- NFC field wake        */
#define TRINITY_RESET_VBUS      (1u << 20)  /* VBUS -- VBUS rising wake     */
...
static uint32_t find_write_offset(void)
{
   ...
   {
      trinity_wdt_kick();   /* feed WDT during scan -- partition may be large */
...
/**
 * \brief  Dump previous session logs -- bench mode only.
 *
 * \details RTT is available immediately at boot. Call before trinity_wdt_init().
 *          Do NOT call in field/USB mode -- UART is not ready this early
 *          and the flash scan may exceed the bootloader WDT window.
 *          Use trinity_log_dump_previous_deferred() for field/USB.
 */
...
/**
 * \brief  Dump previous session logs -- field/USB mode only.
 *
 * \details Call AFTER trinity_wdt_init() and trinity_wdt_kick() so the WDT
 *          is fed before the flash partition scan. UART requires kernel init
 *          to complete before it is usable. trinity_wdt_kick() is called
 *          inside find_write_offset() to prevent WDT expiry during scan.
 */
...
void trinity_log_boot_reason(uint32_t resetreas)
{
   ...
   else if (resetreas & TRINITY_RESET_OFF)     { label = "WAKE_OFF";   }
   else if (resetreas & TRINITY_RESET_VBUS)    { label = "WAKE_VBUS";  }
   else if (resetreas & TRINITY_RESET_DIF)     { label = "WAKE_DIF";   }
   else if (resetreas & TRINITY_RESET_NFC)     { label = "WAKE_NFC";   }
   else if (resetreas & TRINITY_RESET_LPCOMP)  { label = "WAKE_LPCOMP";}
...
int trinity_log_init(void)
{
   ...
   /* Heap-allocate stat buffer -- must not be BSS (crosses .noinit boundary) */
 ...
 *
 * \details The UF2 bootloader arms the WDT before handing off. The peripheral
 *          cannot be stopped or reconfigured once armed:
 *            wdt_install_timeout() -- causes immediate chip reset regardless
 *              of return value (-ENOTSUP/-EBUSY/-EPERM all confirmed).
 *              Confirmed via binary search: adding this call silenced all
 *              UART output before the first printk in field mode.
 *            wdt_setup()           -- same problem, returns -EPERM, resets.
 *            CONFIG_WDT_DISABLE_AT_BOOT -- silently ignored (nRF WDT driver
 *              never selects HAS_WDT_DISABLE_AT_BOOT in Kconfig).
 *
 *          Fix: grab wdt0, assume channel 0, feed immediately, return.
 *          Never call wdt_install_timeout() or wdt_setup(). Applies to
 *          bench, field, and USB modes identically.
 */
