 ...
 *          ble_broadcast() now stamps a uint16_t tx_id sequence counter
 *          into mfg_data[3..4] on every call. See ble_adv.c for details.
 ******************************************************************************/
#ifndef BLE_ADV_H
#define BLE_ADV_H
#include <stdint.h>
 ...
 *         Increments internal s_tx_id counter and stamps it into
 *         mfg_data[3..4] (little-endian) before each update so the hub
 *         can correlate device-side and hub-side logs via tx_id=.
 *
